...
kernel <- "radial"
gamma <- 2^seq(-15, 3, by=3)
cost <- 2^seq(-5, 15, by=3)
grid_search_svm <- tune.svm(damage_grade ~ .,
...
cat("--please now invoke second_part.R importing grid_search_svm$best.parameters----\n")